...
from app.core.exceptions import DuplicateRecordException, ForeignKeyViolationException
...
class TestStudentRepositoryCRUD:
    ...
    async def test_get_all_with_school_filter(self, db_session):
        ...
        await student_repo.create(school_id=school1.id, first_name="John", last_name="Doe")
        await student_repo.create(school_id=school1.id, first_name="Jane", last_name="Smith")
        await student_repo.create(school_id=school2.id, first_name="Bob", last_name="Johnson")
...
class TestStudentAccountStatus:
    ...
    @pytest.mark.asyncio
    async def test_get_account_status_with_multiple_payments_on_invoice(self, db_session, school):
    ...
    async def test_get_account_status_with_mixed_invoices(self, db_session, school):
        ...
        inv1 = next((inv for inv in status['invoices'] if inv['id'] == invoice1.id), None)
        assert inv1['paid_amount'] == Decimal('100.00')
        assert inv1['balance'] == Decimal('0')

        inv2 = next((inv for inv in status['invoices'] if inv['id'] == invoice2.id), None)
        assert inv2['paid_amount'] == Decimal('50.00')
        assert inv2['balance'] == Decimal('150.00')

        inv3 = next((inv for inv in status['invoices'] if inv['id'] == invoice3.id), None)
        assert inv3['paid_amount'] == Decimal('0')
        assert inv3['balance'] == Decimal('150.00')

    @pytest.mark.asyncio
    async def test_get_account_status_preserves_invoice_details(self, db_session, school):
