...
from app.db.models import (AccountMovement, AccountSnapshot, Invoice,
                           InvoiceStatus, MovementEntityType, MovementType,
                           Payment, PaymentMethod, School, SnapshotEntityType,
                           SnapshotType, Student, StudentStatus)
...
from app.workers.account_worker import (handle_invoice_created,
...
@pytest.mark.asyncio
async def test_invoice_service_publishes_event_on_create(
    test_db: AsyncSession, test_student: Student
):
    ...
    # Mock event service
    with patch(
        "app.services.invoice_service.event_service.publish_invoice_created"
    ) as mock_publish:
...
@pytest.mark.asyncio
async def test_invoice_service_publishes_event_on_update(
    test_db: AsyncSession, test_invoice: Invoice, test_student: Student
):
    ...
    # Mock event service
    with patch(
        "app.services.invoice_service.event_service.publish_invoice_updated"
    ) as mock_publish:
        mock_publish.return_value = AsyncMock()
...
@pytest.mark.asyncio
async def test_payment_service_publishes_event_on_create(
    test_db: AsyncSession, test_invoice: Invoice, test_student: Student
):
    ...
    # Mock event service
    with patch(
        "app.services.payment_service.event_service.publish_payment_created"
    ) as mock_publish:
        mock_publish.return_value = AsyncMock()
...
@pytest.mark.asyncio
async def test_worker_handles_payment_created(
...
@pytest.mark.asyncio
async def test_worker_handles_invoice_created(
    test_db: AsyncSession, test_student: Student, test_school: School
):
...
@pytest.mark.asyncio
async def test_get_movements_by_period(
    test_db: AsyncSession, test_student: Student, test_school: School
):
...
async def test_calculate_period_totals(test_db: AsyncSession, test_student: Student):
    ...
    totals = await account_repo.calculate_period_totals(
        MovementEntityType.STUDENT, test_student.id, now - timedelta(days=7), now
    )
...
async def test_snapshot_creation_and_retrieval(
    ...
    retrieved = await account_repo.get_snapshot_at_date(
        SnapshotEntityType.STUDENT, test_student.id, snapshot_date
    )
...
async def test_get_account_status_at_date(test_db: AsyncSession, test_student: Student):
    ...
    status = await account_repo.get_account_status_at_date(
        MovementEntityType.STUDENT, test_student.id, now - timedelta(days=1)
    )
